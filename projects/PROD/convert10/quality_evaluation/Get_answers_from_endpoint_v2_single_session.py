...
def get_answer_from_backend(question, chat_id, user_id):
    try:
        ...
        logging.debug(f"Server response received: Status code={response.status_code}, Content={response.text[:500]}")
        if response.status_code == 200:
            try:
                response_json = response.json()
                message = response_json.get('message', '')
                links = ','.join(response_json.get('links', ['NA']))
                assistant_mode_used = response_json.get('assistant_mode', 'Unknown')
                
                # Log the assistant mode used
                logging.info(f"Assistant mode used: {assistant_mode_used}")
                
                logging.info("Response successfully received from backend.")
        ...
        logging.error(f"Error during network request for /chat_answer: {e}")
        return '', 'NA'
...
def process_csv_and_save_to_xlsx(input_excel_path, output_csv_path, evaluation_results_path):
    try:
        ...
        if not chat_id:
            logging.error("Failed to create a new chat. Exiting process.")
            return
