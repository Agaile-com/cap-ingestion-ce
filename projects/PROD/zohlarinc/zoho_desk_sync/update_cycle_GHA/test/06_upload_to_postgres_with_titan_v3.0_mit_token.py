...
def initialize_bedrock_client():
    """
    Initialisiert den Bedrock Embedding-Client für Amazon Titan unter Verwendung von Dienstname, Region und Modell-ID aus Umgebungsvariablen.

    Returns:
        BedrockEmbeddings: Initialisierter Bedrock Embeddings Client.
    """
    try:
        ...
        logger.error(f"Fehler bei der Initialisierung von Titan Embeddings: {e}", exc_info=True)
        exit(1)

def connect_to_postgres():
    """
    Stellt eine Verbindung zu PostgreSQL her, unter Verwendung der AWS RDS IAM-Authentifizierung.

    Returns:
        SQLAlchemy Engine-Objekt zur Interaktion mit der PostgreSQL-Datenbank.
    """
    client = boto3.client('rds', region_name=REGION)
    token = client.generate_db_auth_token(DBHostname=DB_HOST, Port=DB_PORT, DBUsername=DB_USER, Region=REGION)
    
    # Änderung: Verwenden Sie den tatsächlichen Hostnamen anstelle von 127.0.0.1
...
def process_documents_and_upload_to_pg(docs, embed, engine):
    """
    Bettet Textdaten ein und lädt Metadaten und Embeddings in PostgreSQL hoch.

    Args:
        docs (list): Liste von Dokumentobjekten mit Inhalt und Metadaten.
        embed (BedrockEmbeddings): Initialisierter Embedding-Client.
        engine (SQLAlchemy engine): Verbindung zu PostgreSQL.
    """
    ...
    try:
        ...
        with engine.connect() as connection:
            result = connection.execute(
                text("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'langchain_pg_embedding')")
            ).scalar()
            ...
            if result:
                ...
                logger.info("Table 'public.langchain_pg_embedding' does not exist. Skipping deletion step.")
