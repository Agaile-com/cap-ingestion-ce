...
def initialize_bedrock_client():
    """
    Initialisiert den Bedrock-Client für Amazon Titan.

    Returns:
        boto3.client: Initialisierter Bedrock-Runtime-Client.
    """
    try:
...
def connect_to_postgres():
    logger.debug("Starting connection to PostgreSQL")
    client = boto3.client('rds', region_name=REGION)
    logger.debug(f"Generating DB auth token for host: {DB_HOST}, port: {DB_PORT}, user: {DB_USER}, region: {REGION}")
    token = client.generate_db_auth_token(DBHostname=DB_HOST, Port=DB_PORT, DBUsername=DB_USER, Region=REGION)
    logger.debug(f"DB auth token generated. Token length: {len(token)}")

    connection_string = f"postgresql+psycopg2://{DB_USER}:{quote_plus(token)}@localhost:{DB_PORT}/{DB_NAME}"
    logger.debug(f"Connection string created (password masked): postgresql+psycopg2://{DB_USER}:***@localhost:{DB_PORT}/{DB_NAME}")
